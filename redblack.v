...
Fixpoint size t := match t with E => 0 | T _ tl _ tr => 1 + size tl + size tr end.
...
Ltac do_ins_not_E := 
  repeat match goal with
    ...
    | |- match ?A with E => _ | T _ _ _ _ => _ end  <> _=>destruct A
...
do_compare x t.
congruence.
destruct c; simpl; repeat do_ins_not_E.
destruct c; simpl; repeat do_ins_not_E.
Qed.
...
Inductive searchtree: option key -> option key -> tree -> Prop :=
 | STE: forall lo hi, ltopt lo hi -> searchtree lo hi E
...
Ltac do_searchtree :=
  ...
  |  |- searchtree _ _ (match ?C with E => _ | T _ _ _ _ => _ end) => destruct C
...
Ltac do_interp_balance :=
 ...
 | |- interp (match ?A with E => _ | T _ _ _ _ => _ end) _ => destruct A
 |  H: interp (T _ _ _ _) _ |- _ => inv H
 | H: interp (match ?A with Red => _ | Black => _ end) _ |- _ => destruct A
 | H: interp (match ?A with E => _ | T _ _ _ _ => _ end) _ |- _ => destruct A
 end; auto.
...
| nrRB_r: forall tl k tr n,
         is_redblack tl Black n ->
         is_redblack tr Black n ->
         nearly_redblack (T Red tl k tr) n
| nrRB_b: forall tl k tr n,
         is_redblack tl Black n ->
         is_redblack tr Black n ->
         nearly_redblack (T Black tl k tr) (S n).
...
constructor || 
match goal with 
 | |- nearly_redblack (match ?A with 
                                  | E => _ 
                                  | T _ _ _ _ => _ 
                                  end)  _ =>
        match goal with
        | H: is_redblack A _ _ |- _ => inv H
         | H: nearly_redblack A _ |- _ => inv H
        end
 ...
 | |- nearly_redblack (match K.compare ?A ?B with Eq => _ | Lt => _ | Gt => _ end)  _ =>
                     do_compare A B
...
Ltac jax :=
   repeat
       match goal with 
        ...
        | |- interp match ?t with E => _ | T _ _ _ _ => _ end _ => 
        ...
        | H: interp match ?t with E => _ | T _ _ _ _ => _ end _ |- _  => 
                match t with balance' _ _ _ => fail 1 | append _ => fail 1 | _ =>  destruct t end
        | H: interp (T _ _ _ _) _ |- _ => inv H
...
Program Definition empty : t := E.
 Next Obligation. split. constructor; simpl; auto. do 2 econstructor.
 Qed.

Definition is_empty (x: t) : bool := 
   match proj1_sig x with E => true | _ => false end.

Definition mem (k: key) (x: t) : bool := member k (proj1_sig x).

Program Definition add (k: key) (x: t) : t := insert k (proj1_sig x).
 Next Obligation.
  destruct x as [x [? ?]].
  split.  
...
Program Definition singleton (k: key) : t := T Black E k E.
  Next Obligation.
   split. repeat constructor; auto.
   exists 1; repeat constructor; auto.
  Qed.

Program Definition remove (k: key) (x: t) : t := delete k (proj1_sig x).
 Next Obligation.
  destruct x as [x [? ?]].
  split. 
  unfold delete; intros.
  simpl; apply (del_is_searchtree k) in s.
  inv s; constructor; auto.
   simpl.
   destruct e. 
   eapply delete_is_redblack; simpl; eauto.
 Qed.
...
Lemma interp_compare': forall a b, 
 ...
 Proof.
  ...
  destruct t1_1; try contradiction.
  do_compare t0 t1; try discriminate.
  simpl in H3, H4.
...
Lemma compare'_interp_aux: 
 ...
 Proof.
  ...
  destruct (H2 t0). 
  assert (interp E t0). apply H3. constructor 1; auto. reflexivity. inv H4.
  generalize (shape_gopher a) as SHa.
  case_eq (gopher a); intros.
  destruct a; inv H3.
  replace b with E. simpl; auto.
  destruct b; auto.
  destruct (H2 t0).
  assert (interp E t0). apply H4. constructor 1; auto. reflexivity. inv H5.
  destruct (gopher a1); inv H5.
  destruct t0; try contradiction. clear SHa.
  ...
  destruct gb.
  specialize (IGa t1). specialize (H6 t1). specialize (H2 t1).
  rewrite <- H2 in H6.
  rewrite <- H6 in IGa.
  clear - IGa.
  assert (interp E t1); [| inv H].
  ...
  apply IHn with (lo:=Some t1)(hi:=hi)(lo':=Some t0)(hi':=hi'); auto.
  clear - H H4. omega.
  ...
  inv H22.
  auto.
  rename t0 into j;   rename t1 into k.
...
Function linear_union_aux (st : tree*tree) (nl: positive * list key)
 ...
 | E , E => nl
 | T _ sl j E as st1 , T _ tl j' E as st2 => 
    match K.compare j j' with
 ...
 | T _ sl j E, E => linear_union_aux (sl,E) (Psucc (fst nl), j:: snd nl)
 |  E, T _ tl j' E => linear_union_aux (E,tl) (Psucc (fst nl), j':: snd nl)
 | _, _ => nl (* impossible *)
 end.
 Proof. 
  simpl; intros; subst;
...
Function linear_diff_aux (st : tree*tree) (nl: positive * list key)
 ...
 | T _ sl j E, E => linear_diff_aux (sl,E) (Psucc (fst nl), j:: snd nl)
 | _, _ => nl (* impossible *)
 end.
 Proof. 
  simpl; intros; subst;
...
Fixpoint compare_height (s1x s1 s2 s2x: tree) : comparison :=
 ...
 | _, E, _, T _ _ _ _ => Lt
 | T _ _ _ _, _, E, _ => Gt
 | T _ s1x' _ _, T _ s1' _ _, T _ s2' _ _, E => 
        compare_height (skip_black s1x') s1' s2' E
 | E, T _ s1' _ _, T _ s2' _ _, T _ s2x' _ _ => 
        compare_height E s1'  s2'  (skip_black s2x')
 | _, _, _, _ => Eq
 end.

Definition fold (A: Type) (f: key -> A -> A) (x: t) (base: A) : A :=
  fold' A f (proj1_sig x) base.


Definition for_all (f: key -> bool) (x: t) : bool := 
   fold bool (fun k y => andb (f k) y) x true.

Definition exists_ (f: key -> bool) (x: t) : bool := 
...
Lemma treeify_g_induc:
       ...
       (forall x l1, fP xH (x :: l1) (T Red E x E, l1)) ->
       (forall  l n' t1 x l2 t2 l3
           (LEN: length l >= nat_of_P n')
         (LEN2: S (length l2) >= nat_of_P n'),
          fP n' l (t1, x :: l2) -> treeify_f n' l = (t1, x :: l2) -> gP n' l2 (t2, l3) ->
...
Lemma treeify_f_not_E:
  forall n l, length l >= nat_of_P n -> fst (treeify_f n l) <> E.
Proof.
 induction n; simpl; intros; 
  try rewrite nat_of_P_xH in *;
  try rewrite nat_of_P_xO in *;
...
Lemma treeify'_g_is_searchtree:
 ...
 unfold fP,gP; clear fP gP; intros;
  ...
  split; [|split]; auto.
  constructor; auto.
  apply treeify_f_not_E in LEN; rewrite H0 in LEN. simpl in LEN.
  destruct t1; try solve [contradiction LEN; auto]. inv H5.
    apply treeify_f_not_E in LEN2; rewrite H2 in LEN2; simpl in LEN2.
...
Definition Kle x y := K.lt x y \/ K.eq x y.
 ...
 Proof. 
  ...
  destruct IHs2 as [? [? [? ?]]].
  remember (f t0) as ft0; destruct ft0.
  ...
  specialize (H7 COMPAT x); clear - COMPAT Heqft0 H7; intuition etac. inv H1; auto. inv H3; auto.
  ...
  intuition.
  inv H7; auto.
  rewrite (COMPAT _ _ H22) in *. rewrite <- Heqft0 in H17; inv H17.
Qed.
 
Program Definition filter (f: key -> bool) (s: t) : t := 
  let nl := filter_aux f (proj1_sig s) 1%positive nil
   in fst (treeify_g (fst nl) (snd nl)).
 Next Obligation.
  destruct s as [s ?].
  simpl.
  case_eq (filter_aux f s 1 nil); intros n l ?.
  destruct v.
  destruct (filter_aux_lemma _ _ _ _ _ _ H0 H) as [? [? [? _]]].  
  ...
  apply searchtree_expand_right with t0; simpl; auto.
  eauto.
Qed.
...
remember (rgopher s2) as s2'.
destruct s1'.
assert (s1=E) by (destruct s1; simpl in Heqs1'; try destruct (rgopher s1_2); inv Heqs1'; auto).
subst s1; clear Heqs1'.
destruct s2'.
assert (s2=E) by (destruct s2; simpl in Heqs2'; try destruct (rgopher s2_2); inv Heqs2'; auto).
...
specialize (IHN E s2'1).
spec IHN.
  generalize (size_rgopher s2); rewrite <- Heqs2'; simpl; omega.
rewrite searchtree_rgopher in H1.
rewrite <- Heqs2' in H1; inv H1.
assert (searchtree lo (Some t0) E). constructor. eapply searchtree_proper_bounds; eauto.
...
rewrite searchtree_rgopher in H0.
rewrite <- Heqs1' in H0.  inv H0.
assert (searchtree lo (Some t0) E) by (apply searchtree_proper_bounds in H11; constructor; auto).
specialize (IHN _ _ H11 H0).
destruct (IHN _ _ _ _ H2) as [? [? ?]]; simpl; auto; clear IHN.
constructor; auto. right; reflexivity.
constructor; auto.
...
destruct lo; simpl; auto. destruct hi. apply HdRel_le_lt with t1; auto. destruct H1; auto. rewrite H0; simpl; auto.
 destruct lo; simpl; auto. destruct hi. apply HdRel_le_lt with t1; auto. destruct H1; auto. rewrite H1; simpl; auto.
simpl in H1.
...
Lemma partition_aux_lemma:
  forall f, compatb f ->
   forall s lo hi, searchtree lo hi s -> 
     forall nl1' nl2' nl1 nl2,
           (nl1',nl2') = partition_aux f s nl1 nl2 ->
      (hi=None -> snd nl1 = nil /\ snd nl2 = nil) ->
     HdRel' Kle hi (snd nl1) ->
     ...
     S (length (snd nl1')) = nat_of_P (fst nl1') /\
     S (length (snd nl2')) = nat_of_P (fst nl2') /\
     forall x, (InA K.eq x (snd nl1') <-> f x = true /\ interp s x \/ InA K.eq x (snd nl1)) /\
                  (InA K.eq x (snd nl2') <-> f x = false /\ interp s x \/ InA K.eq x (snd nl2)).
Proof.
intros f COMPAT.
induction 1; intros.
simpl in H0. inv H0. intuition.
...
simpl in H22.
intro x; specialize (H22 x); specialize (H15 x).
clear - COMPAT H22 H15 H H0 Heqb.
intuition.
inv H2.
rewrite (COMPAT _ _ H15) in *.
intuition.
destruct (H5 H15); auto.
destruct H2; left; split; auto.
inv H15; intuition.
inv H15; intuition.
rewrite (COMPAT _ _ H20) in *.
rewrite <- Heqb in H4. inv H4.
intro Hx; inv Hx.
simpl in *.
apply HdRel_lt_le. auto.
constructor; auto. right; reflexivity.
...
rewrite (COMPAT _ _ H15) in *.
left; split; auto.
destruct (H1 H15); auto.
destruct H3; left; split; auto.
inv H15; intuition.
Qed.

Program Definition partition (f: key -> bool) (s: t) : t*t := 
  match partition_aux f (proj1_sig s) (1%positive, nil) (1%positive,  nil)
  with ((n1,l1),(n2,l2)) =>
        (fst (treeify_g n1 l1), fst (treeify_g n2 l2))
  end.
 Next Obligation.
  destruct s as [s [?H ?H]].
  simpl in *.
  destruct (partition_aux_lemma1 f _ _ _ H _ _ _ _ Heq_anonymous)
  as [? [? [? [? [? ?]]]]]; simpl; auto.
  simpl in *.
...
Definition union (t1 t2: t) : t := 
  let s1 := proj1_sig t1
  in let s2 := proj1_sig t2
  in match compare_height s1 s1 s2 s2 with
      | Lt => fold t add t1 t2
      | Gt => fold t add t2 t1
      | Eq => exist valid _ (linear_union_valid _ _ (proj2_sig t1) (proj2_sig t2))
    end.
...
Definition diff (t1 t2: t) : t := 
  ...
  in match compare_height s1 s1 s2 s2 with
      ...
      | Gt => fold t remove t2 t1
      | Eq => exist valid _ (linear_diff_valid _ _ (proj2_sig t1) (proj2_sig t2))
    end.
...
Definition inter (t1 t2: t) : t := 
  ...
  in match compare_height s1 s1 s2 s2 with
      ...
      | Eq => exist valid _ (linear_inter_valid _ _ (proj2_sig t1) (proj2_sig t2))
    end.

Definition elements (x: t) : list key := 
   List.rev (fold (list key) (@cons key) x nil).

Definition choose (x: t) : option key := choose' (proj1_sig x).
Definition max_elt (x: t) : option key := max_elt' (proj1_sig x).

Definition elt := key.

Definition equal (t1 t2 : t) : bool := 
  match compare' (proj1_sig t1) (proj1_sig t2) with
  | Eq => true
  | _ => false
  end.
...
Qed.
 ...
 Qed.
  ...
  Proof. unfold Empty, empty, In, mem. intros. intro.  simpl in H.
   inv H.
  Qed.

  Lemma is_empty_spec : forall s, is_empty s = true <-> Empty s.
  Proof.
   unfold is_empty, Empty, In, mem.
 ...
 induction s; intros; auto.
  ...
  Lemma fold'_spec:  forall s (A : Type) (i : A) (f : elt -> A -> A),
 ...
 remember (fold' (list key) (@cons key) s1 (@nil key)) as l1.
  change (cons t0 l1) with (app nil (cons t0 l1)).
  rewrite fold'_app.
 ...
 Qed.
  ...
  Proof.
    intros.
     unfold elements, In, mem.
    rewrite SetoidList.InA_rev.
   2: apply E.eq_equiv.
   destruct s as [s [? ?]]. unfold fold. simpl.
   clear.
    induction s; simpl.
   intuition; inv H.
   intuition.
   change (fold' _ _ _ _) with 
     (fold' (list key) (@cons _) s2 ((t0 :: nil) ++ fold' (list key) (@cons _) s1 nil)%list)
    in H3.
    rewrite fold'_app in H3.
   apply SetoidList.InA_app  in H3.
   destruct H3.
   change (cons t0 nil) with (app nil (cons t0 nil)) in H3.
   rewrite fold'_app in H3.
   apply SetoidList.InA_app  in H3.
   destruct H3.
   constructor 3. auto.
   constructor 1. inv H3; auto. inv H5.
  apply E.eq_equiv.
   constructor 2. auto.
  apply E.eq_equiv.
  change (cons t0 (fold' (list key) (@cons _) s1 nil))
    with (app nil (app (cons t0 nil) (fold' (list key) (@cons _) s1 nil))).
  repeat rewrite fold'_app.
  repeat  rewrite -> SetoidList.InA_app_iff by apply E.eq_equiv.
  inv H3. right; left. constructor; auto.
  right; right; auto.
  left; auto.
 Qed.
  ...
  Proof.
   intros. destruct s as [s [v ?]].
   unfold elements, fold; simpl.
   ...
   simpl List.rev.
   destruct H as [lo [hi H]]. inv H.
   apply SetoidList.NoDupA_app; try apply E.eq_equiv; eauto.
   apply SetoidList.NoDupA_app; try apply E.eq_equiv; eauto.
   constructor. intro Hx; inv Hx.
   constructor.

    intros.
    inv H0; [| inv H2].
    rewrite H2 in H.
    destruct (InA_rev_fold'_range _ _ _ _ H4 H).
    contradiction (lt_irrefl H1).
   
   intros.
   rewrite -> SetoidList.InA_app_iff in H by apply E.eq_equiv.
   destruct H.
   do_compare x t0.
   rewrite e in H.
   destruct (InA_rev_fold'_range _ _ _ _ H4 H).
   contradiction (lt_irrefl H2).
   destruct (InA_rev_fold'_range _ _ _ _ H7 H0).
   simpl  in H1. clear - l H1.
   contradiction (lt_irrefl (lt_trans l H1)).
   destruct (InA_rev_fold'_range _ _ _ _ H4 H).
   simpl in H2.
   contradiction (lt_irrefl (lt_trans l H2)).
   inv H. rewrite H2 in H0.
   destruct (InA_rev_fold'_range _ _ _ _ H7 H0).
   simpl in H. contradiction (lt_irrefl H). 
   inv H2.
 Qed.
...
Qed.
  ...
  Proof.
  intros; unfold union.
  destruct (compare_height (proj1_sig s) (proj1_sig s) (proj1_sig s') (proj1_sig s')).
 (* case 1 *)
  destruct s as [s [?H ?]]; destruct s' as [s' [?H ?]]; unfold In, mem,  union, linear_union; simpl.
  clear e e0.
 ...
 (* case 2 *)
  rewrite fold_spec.
  rewrite <- (elements_spec1 s x).
  remember (elements s) as el; clear s Heqel.
  change (fun (a : t) (e : elt) => add e a) with (flip add).
  induction el; simpl. intuition etac.
  transitivity (K.eq x a \/ (InA E.eq x el \/ In x s')).
  rewrite <- IHel; clear IHel.
  unfold flip at 2.
  revert s';   induction el; simpl; intros.
  rewrite add_spec; intuition.
  rewrite <- IHel. unfold flip at 2 4.
  generalize (add_commute x a0 a s'); intro.
  remember (add a0 (add a s')) as u; remember (add a (add a0 s')) as v.
  clear - H; revert u v H; induction el; simpl; intros; auto.
  apply IHel. unfold flip; repeat rewrite add_spec; intuition.
  clear; intuition. inv H0; intuition.
 (* case 3 *)
  rewrite fold_spec.
  change (fun (a : t) (e : elt) => add e a) with (flip add).
  generalize (elements_spec1 s x); intro.
  rewrite <- (elements_spec1 s' x).
  remember (elements s') as el; clear s' Heqel.
  revert s H; induction el; simpl; intros. intuition etac.
  transitivity (K.eq x a \/ (In x s \/ InA E.eq x el)).
  rewrite <- IHel; clear IHel.
  unfold flip at 2.
  revert s H;   induction el; simpl; intros.
  rewrite add_spec; intuition.
  rewrite <- IHel. unfold flip at 2 4.
  generalize (add_commute x a0 a s); intro.
  remember (add a0 (add a s)) as u; remember (add a (add a0 s)) as v.
  clear - H H0; revert u v H H0; induction el; simpl; intros; auto.
  apply IHel; auto. unfold flip; repeat rewrite add_spec; intuition.
  unfold flip.
  apply elements_spec1.
  auto.
  intuition. inv H2; intuition.
Qed.
 ...
 Lemma filter_spec : forall s x f,
  ...
  Proof.
  intros [s [? ?]] ? ? ?.
  unfold In, mem, filter. simpl.
 clear e.
 remember (filter_aux f s 1 nil) as nl.
  destruct nl as [n l].
  destruct (filter_aux_lemma _ _ _ _ _ _ s0 (Logic.eq_sym Heqnl)) as [? [? [? ?]]].
  destruct (treeify'_g_is_searchtree n l None) as [? [? ?]]; simpl; auto.
  omega.
  rewrite (H3 H x).
  rewrite <- (H5 x).
  ...
  spec H7; [omega|].
  rewrite H2 in H7.
  destruct (snd (treeify_g n l)); [ | elimtype False; simpl in H7; clear - H7; omega].
  clear; intuition etac.
Qed.

Lemma mem_compat:
  forall s, Proper (E.eq ==> Logic.eq) (fun k : key => mem k s).
Proof.
  repeat intro.
  generalize (In_compat x y H s s (refl_equal _)); intro.
  repeat rewrite <- mem_spec in H0.
...
Qed.
  ...
  destruct (compare_height (proj1_sig s) (proj1_sig s) (proj1_sig s') (proj1_sig s')).
 (* case 1 *)
  destruct s as [s [?H ?]]; destruct s' as [s' [?H ?]]; unfold In, mem,  inter, linear_inter; simpl.
  clear e e0.
  ...
  clear - H5 H8; intuition.
 (* case 2 *)
  rewrite filter_spec; [  | apply mem_compat].
  rewrite mem_spec. intuition.
 (* case 3 *)
  rewrite filter_spec; [  | apply mem_compat].
  rewrite mem_spec. intuition.
Qed.
...
Qed.
 ...
 Lemma diff_spec : forall s s' x, In x (diff s s') <-> In x s /\ ~In x s'.
  ...
  destruct (compare_height (proj1_sig s) (proj1_sig s) (proj1_sig s') (proj1_sig s')).
 (* case 1 *)
  destruct s as [s [?H ?]]; destruct s' as [s' [?H ?]]; unfold In, mem, linear_diff; simpl.
  clear e e0.
 ...
 (* case 2 *)
  rewrite filter_spec.
  unfold negb.
  generalize (mem_spec s' x); intro.
  destruct (mem x s'); intuition.
  repeat intro. f_equal. eapply mem_compat; eauto. 
 (* case 3 *)
  rewrite fold_spec.
  change (fun (a : t) (e : elt) => remove e a) with (flip remove).
  generalize (elements_spec1 s x); intro.
  rewrite <- (elements_spec1 s' x).
  remember (elements s') as el; clear s' Heqel.
  revert s H; induction el; simpl; intros. intuition etac.
  ...
  rewrite <- IHel. unfold flip at 2 4.
  generalize (remove_commute x a0 a s); intro.
  remember (remove a0 (remove a s)) as u; remember (remove a (remove a0 s)) as v.
  clear - H H0; revert u v H H0; induction el; simpl; intros; auto.
  apply IHel; auto. unfold flip; repeat rewrite remove_spec; intuition.
  unfold flip.
  apply elements_spec1.
  auto.
  intuition. inv H0; intuition.
Qed.
 ...
 Qed.
  ...
  Proof.
   ...
   rewrite <- List.rev_length.
    unfold elt.
    remember (List.rev (elements s)) as  l; clear.
...
Lemma partition_aux_filter:
 ...
 Qed.
  ...
  Proof.
  intros [s [?H ?H]] ? ? ?.
  unfold In, filter, partition; simpl.
    remember (partition_obligation_1 f (exist valid s (conj H H0))) as H3.
  clear HeqH3.
  remember (partition_obligation_2 f (exist valid s (conj H H0))) as H4.
  clear HeqH4.
 assert (filter_aux f s 1 nil = fst ( partition_aux f s (1%positive, nil) (1%positive, nil)))
 ...
 Proof.
  ...
  remember (partition_obligation_2 f (exist valid s (conj H H0))) as H4.
  clear HeqH4.
 assert (filter_aux (compose negb f) s 1 nil = snd ( partition_aux f s (1%positive, nil) (1%positive, nil)))
   by (rewrite partition_aux_filter; simpl; auto).
  unfold compose in H2; rewrite H2. simpl in *.
  clear.
  destruct (partition_aux f s (1%positive, nil) (1%positive, nil)).
  destruct p as [n1 l1]. destruct p0 as [n2 l2]. simpl.
  intuition.
 Qed.
  ...
  Proof.
   ...
   Lemma choose'_spec2 : forall s, choose' s = None -> s=E.
   Proof.
     induction s; simpl; intros; auto.
   ...
   Proof.
     intros.
    destruct s as [s [? ?]]; unfold choose, Empty, In in *;
     simpl in *.
...
Lemma lt_strorder : StrictOrder lt.
 ...
 (* Transitive *)
  ...
  destruct x as [a ?]; destruct y as [b ?]; destruct z as [c ?]; simpl in *.
  clear - H H0.
  remember (size c).
  ...
  destruct (gopher b); inv H0. destruct t0_1; inv H2.
  ...
  do_compare t1 t2; try solve [inv H].
  rewrite e.
  do_compare t2 t0; auto.
  apply (IHn a'2 b'2 c'2); auto.
  rewrite <-  Heqc' in H2; simpl in H2; omega.
   do_compare t2 t0; auto.
  rewrite e in l. rewrite <- compare_lt_iff in l. rewrite l. auto.
  replace (K.compare t1 t0) with Lt; auto.
  symmetry. rewrite compare_lt_iff. apply lt_trans with t2; auto.
  inv H0.
Qed.

Lemma tree_similar_lemma':
       forall c c' k k' s s' lo hi lo' hi',
          searchtree lo hi (T c E k s) ->
          searchtree lo' hi' (T c' E k' s') ->
          (forall x : key,
            interp (T c E k s) x <-> interp (T c' E k' s') x) ->
...
Lemma lt_compat: Proper (eq ==> eq ==> iff) lt.
 ...
 (*  PART 1 *)
  ...
  unfold eq, Equal, lt, In, compare, mem in *.
  ...
  revert a' H loa' hia' STa' a loa hia STa
   ...
   2: elimtype False; clear - Gab; destruct (Gab t1);
        absurd (interp E t1); [intro Hx; inv Hx 
                                     | apply H0; constructor 1; reflexivity].
   destruct (gopher b'); auto.
    absurd (interp E t0); [intro Hx; inv Hx 
...
Lemma elements_spec2 : forall s, Sorted.sort E.lt (elements s).
 Proof.
  ...
  assert (forall x, 
             ...
             exists lo', lo = Some lo' /\ (K.eq x lo' \/ K.lt x lo')).
  subst rest; intros; simpl in *. inv H0.
  assert (Sorted.Sorted K.lt (List.rev rest)). subst rest; constructor.
  clear Heqrest.
  revert lo hi H rest H0 H1; 
      induction s; simpl; intros.
  apply H1.
  change (Sorted.Sorted E.lt
  (List.rev
     (fold' (list key) (@cons _) s2 (nil ++ (t0::nil) ++ fold' (list key) (@cons _) s1 rest)%list))).
  repeat rewrite fold'_app.
  repeat rewrite List.rev_app_distr.
  inv H.
  apply SetoidList.SortA_app with K.eq.
  apply E.eq_equiv.
  ...
  repeat constructor.
  intros.
  change rest with (app nil rest) in H.
  rewrite fold'_app in H.
  rewrite List.rev_app_distr in H; apply SetoidList.InA_app in H.
  destruct H.
  rewrite SetoidList.InA_rev in H. 
  destruct (H0 _ H) as [lo' [? ?]]. subst lo. 
  apply searchtree_proper_bounds in H6; simpl in H6.
  inv H2. rewrite H5. 
  destruct H4. rewrite H2. auto. apply lt_trans with lo'; auto. inv H5.
   apply E.eq_equiv.
   inv H2. rewrite H4.
   destruct (InA_rev_fold'_range _ _ _ _ H6 H).
   simpl in H3. auto. inv H4.
   apply E.eq_equiv.
   eapply IHs2; try eassumption.
   intros. inv H. constructor.
   intros.
   apply SetoidList.InA_app in H.
  destruct H.
   change rest with (app nil rest) in H.
  rewrite fold'_app in H.
  rewrite List.rev_app_distr in H; apply SetoidList.InA_app in H.
  destruct H.
  destruct (InA_rev_fold'_range _ _ _ _ H9 H2).
  apply lt_trans with t0; auto.
  rewrite SetoidList.InA_rev in H. 
  destruct (H0 _ H) as [lo' [? ?]]. subst lo.
  apply searchtree_proper_bounds in H6; simpl in H6.
  destruct H7. rewrite H5; auto. apply lt_trans with lo'; auto.
   apply E.eq_equiv.
     destruct (InA_rev_fold'_range _ _ _ _ H6 H).
  destruct (InA_rev_fold'_range _ _ _ _ H9 H2).
  apply lt_trans with t0; auto.
...
Proof.
 ...
 assert (fst (delmin s1 t0 s2) = x).
 clear - H. 
 revert t0 s2 H; induction s1; simpl; intros.
 inv H; auto.
 destruct s1_1. inv H.
 specialize (IHs1_1 x s1_2 (refl_equal _)).
  destruct (delmin E x s1_2); auto. destruct c0; auto.
  specialize (IHs1_1 t0 s1_2 H).
  destruct (delmin (T c1 s1_1_1 t2 s1_1_2) t0 s1_2); subst.
  destruct c0; auto.
  assert (snd (delmin s1 t0 s2) = del x (T c s1 t0 s2)).
  clear - H ST.
  revert t0 s2 c H lo hi ST.
  induction s1;  intros. inv H. simpl. rewrite compare_refl.
  rewrite append_equation; auto.
  destruct s1_1.
  inv H.  simpl.  
  pattern (append(E,s1_2)); rewrite append_equation.
  rewrite compare_refl. 
  inv ST.
  replace (K.compare x t1) with Lt.
  destruct c; auto.
  inv H3. clear - H8. apply searchtree_proper_bounds in H8; simpl in H8.
  symmetry; rewrite compare_lt_iff; auto.
  inv ST.
  specialize (IHs1_1 t0 s1_2 c H _ _ H4).
  revert IHs1_1; case_eq (delmin (T c1 s1_1_1 t2 s1_1_2) t0 s1_2); intros.
  remember (T c1 s1_1_1 t2 s1_1_2) as B.
  simpl snd in IHs1_1.
  ...
  apply lt_trans with t2; auto.
  eapply searchtree_None; eauto.
  assert (K.compare x t1 = Lt).
  clear - H4 H H7.
   assert (choose' (T c B t0 s1_2) = Some x).
  ...
  destruct c; subst t4; auto.
  rewrite <- H1. destruct (delmin s1 t0 s2); subst; auto.
  destruct t2; auto.
 Qed.
...
Lemma delete_min_valid:
       forall s: t, match delete'_min (proj1_sig s) with
                        | Some (_, s0) => valid s0
                        | None => True
                       end.
 Proof.
  intros. 
 case_eq (delete'_min (proj1_sig s)); intros; auto.
 destruct p as [k s'].
  destruct s as [s [? ?]].
  simpl in *.
  case_eq (choose' s); intros.
  rewrite (delmin_eq s t0) in H; auto.
  inv H.
  split.
  apply (del_is_searchtree k) in s0.
  unfold delete. destruct (del k s); simpl; auto. inv s0; repeat constructor; auto.
  destruct e as [n ?]; apply delete_is_redblack with n; auto.
  apply choose'_spec2 in H0. subst s. simpl in *. discriminate.
Qed.

Definition delete_min (s: t) : option (elt * t) :=
  opt_t_aux (delete'_min (proj1_sig s)) (delete_min_valid s).
 ...
 Proof.
  ...
  remember (delete_min_valid (exist valid s v)) as P.
  clear HeqP. simpl in P.
  split; intro.
  case_eq (choose' s); intros. 
   generalize (delmin_eq _ _ (proj1 v) H0); intro.
   destruct (delete'_min s); inv H1.
   simpl in H. inv H. split; simpl; auto.
   rewrite (proof_irrelevance _ v0 (remove_obligation_1 k (exist valid s v))).
   auto.
